...
Citizen.CreateThread(function()
  while true do
    local hour = string.sub(tostring(GetClockHours()), 1)
    local hour2 = string.sub(tostring(GetClockHours()), 2)
    local hourFinales = GetClockHours()
    local minutes = string.sub(tostring(GetClockMinutes()), 1)
    local minutes2 = string.sub(tostring(GetClockMinutes()), 2)
    local minutesFinales = GetClockMinutes()
    if minutes2 == '' then
      minutesFinales = tostring('0' .. minutes)
    end
    if hour2 == '' then
      hourFinales = tostring('0' .. hour)
    end
    Citizen.Wait(100)
    timeString = (hourFinales .. ':' .. minutesFinales)
    Citizen.Wait(1000)
  end
end)
...
if hasFocus == true then
  ...
  elseif IsControlJustPressed(1, 176) then -- TAKE.. PIC
    exports['screenshot-basic']:requestScreenshotUpload("https://discord.com/api/webhooks/835869077679243276/MHfPZPwXlk4BHp0pi5l96dGVR8_PKFKHbo_Vf8oAfPm0FTey6Xxa73HaR0kC2THvftqk", data.field, function(data)
